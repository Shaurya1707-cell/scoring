...
import { recordPoint } from "@/lib/scoringEngine";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { matchId, gameId, scoringTeamId } = await request.json();

    if (!matchId || !gameId || !scoringTeamId) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
    }

    // Call scoring engine
    const state = await recordPoint({
      matchId,
      gameId,
      scoringTeamId,
      userId: session.id,
    ...
    console.error("API referee score error:", err);
